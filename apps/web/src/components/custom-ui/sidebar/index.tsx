...
function Sidebar({ className, isCollapsed = false, onToggleCollapse }: SidebarProps) {
  ...
  return (
    <TooltipProvider>
      ...
      >
        {/* Toggle Button */}
        <div className={`flex ${isCollapsed ? 'justify-center mb-2' : 'justify-end mb-2'}`}>
          <Tooltip>
        ...
        {/* Avatar Section */}
        <div className={`flex gap-4 p-2 mb-4 ${isCollapsed ? 'justify-center' : ''}`}>
          <Tooltip>
            <TooltipTrigger asChild>
              <Avatar className="w-[50px] h-[50px]">
                <AvatarImage src="https://github.com/shadcn.png" alt="@shadcn" />
                <AvatarFallback>
                  {profileInfo?.name ? profileInfo.name.substring(0, 2).toUpperCase() : "CN"}
                </AvatarFallback>
              </Avatar>
            </TooltipTrigger>
            {isCollapsed && (
              <TooltipContent side="right">
                <div className="text-center">
                  <p className="font-semibold">{profileInfo?.name ?? "No Name"}</p>
                  <p className="text-xs opacity-75">{profileInfo?.role ?? "No Role"}</p>
                </div>
              </TooltipContent>
            )}
          </Tooltip>
          ...
          {!isCollapsed && (
            <div className="flex-1 flex flex-col justify-center overflow-hidden">
              <h1 className="font-bold p-0 truncate">{profileInfo?.name ?? "No Name"}</h1>
