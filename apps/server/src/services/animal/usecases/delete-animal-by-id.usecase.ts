...
import { AnimalModel, AnimalHealthRecordModel, DietPlanModel } from 'src/database';
...
export class DeleteAnimalByIdUseCase {
  constructor(
    ...
    @InjectModel('AnimalHealthRecord') private readonly animalHealthRecordModel: AnimalHealthRecordModel,
  ) {}
  ...
  async execute(animalId: string): Promise<AnimalDeleteResponse> {
    ...
    try {
      ...
      throw new Error(`Failed to delete animal and related records: ${error.message}`);
    } finally {
